...
{
      /*SETBIT(RCGCGPIO,port); //bya5od rkm al port
      SETBIT(RCC,11);//bypass The system clock is derived from the OSC source and divided by the divisor specified by SYSDIV.
       SETBIT(RCC,13);//The PLL is powered down
       CLRBIT(RCC,22);//The system clock is used undivided
       //piosc use internal oscllator
       SETBIT(RCC,4);
        CLRBIT(RCC,5);
        */
  SETBIT(RCGCGPIO,5);
